...
@section('content')
    <div class="col-xl-10 mx-auto {{ app()->getLocale('fa') ? 'd-rtl' : '' }}">        <div class="card">
            <div class="card-header px-4 py-3">
                <h5 class="mb-0">{{ __('create_new_category') }}</h5>
            </div>
            <div class="card-body p-4">
                <form method="POST" action="{{ route('articles.store') }}" class="row g-3 needs-validation"
                    ...
                    <div class="col-md-12">
                        ...
                        <textarea type="text" class="form-control resize-none" name="description" id="description"
                            ...
                            placeholder="{{ __('enter_short_description') }}" required=""></textarea>
                    </div>
                    @error('description')
                        <small class="text text-danger"><b>{{ $message }}</b></small>
                    @enderror
                    ...
                    <div class="col-md-12">
                        ...
                        <textarea type="text" class="form-control resize-none" name="content" id="content"
                            value="{{ old('content', optional($article ?? null)->content) }}" cols="5" rows="5"
                            placeholder="{{ __('enter_content_of_article') }}" required=""></textarea>
                    </div>
                    @error('content')
                        <small class="text text-danger"><b>{{ $message }}</b></small>
                    @enderror
