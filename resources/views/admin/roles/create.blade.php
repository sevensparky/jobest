...
@section('title', __('admin_panel_create_role'))
@section('content')
<div class="col-xl-10 mx-auto {{ app()->getLocale('fa') ? 'd-rtl' : '' }}">
    <div class="card">
        <div class="card-header px-4 py-3">
            <h5 class="mb-0">{{ __('create_role') }}</h5>
        </div>
        <div class="card-body p-4">
            <form method="POST" action="{{ route('roles.store') }}" class="row g-3 needs-validation" novalidate="">
                ...
                <div class="col-md-12">
                    ...
                    <textarea class="form-control resize-none" name="description" id="description" placeholder="{{ __('example_writer') }}" cols="3" rows="3"></textarea>
                    @error('description')
                        <small class="text text-danger"><b>{{ $message }}</b></small>
                    @enderror
                </div>
