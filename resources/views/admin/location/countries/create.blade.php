...
@section('title', __('admin_panel_create_country'))
@include('common.alerts.alert')
@section('content')
    <div class="col-xl-10 mx-auto {{ app()->getLocale('fa') ? 'd-rtl' : '' }}">
        <div class="card">
            <div class="card-header px-4 py-3">
                <h5 class="mb-0">{{ __('create_country') }}</h5>
            </div>
            <div class="card-body p-4">
                <form method="POST" action="{{ route('countries.store') }}" class="row g-3 needs-validation" novalidate="">
                    @csrf
                    <div class="col-md-12">
                        <label for="name" class="form-label">{{ __('name') }}</label>
                        <input type="text" class="form-control" name="name" id="name"
                            placeholder="{{ __('enter_name_of_country') }}" required="">
                    </div>
                    @error('name')
