...
@section('title', __('admin_panel_edit_industry-type'))
@section('content')
    <div class="col-xl-10 mx-auto {{ app()->getLocale('fa') ? 'd-rtl' : '' }}">
        <div class="card">
            <div class="card-header px-4 py-3">
                <h5 class="mb-0">{{ __('edit_industry_type') }}</h5>
            </div>
            <div class="card-body p-4">
                <form method="POST" action="{{ route('industry-type.update', $item->id) }}" class="row g-3 needs-validation"
                    novalidate="">
                    @csrf
                    @method('PUT')
                    <div class="col-md-12">
                        ...
                        <input type="text" class="form-control" name="name" id="name"
                            value="{{ $item->name }}" placeholder="{{ __('enter_name_of_industry_type') }}" required="">
                    </div>
                    @error('name')
